...
const T = {
  ru: {
    systemBot:   name => `Ты умный и дружелюбный Telegram-бот NexusBot_777. Отвечай ТОЛЬКО на русском языке. Будь полезным, кратким и позитивным. Используй эмодзи. Пользователя зовут ${name}. Не упоминай, что ты ИИ.`,
    systemSearch: name => `Ты помощник NexusBot_777. Отвечай ТОЛЬКО на русском языке. Пользователя зовут ${name}. Составь краткий ответ на основе результатов поиска. Используй эмодзи.`,
    systemVision: caption => caption ? `Опиши фото. Учти подпись: "${caption}"` : 'Подробно опиши что изображено на фото на русском языке.',
